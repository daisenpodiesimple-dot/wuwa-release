...
try {
  if (typeof $.fn.draggable === "undefined") {
    import("https://testingcf.jsdelivr.net/npm/jquery-ui/ui/widgets/draggable/+esm").then(
      () => console.log("JQUI Draggable Loaded"),
    );
  }
} catch (e) {
  console.error("Auto-import draggable failed:", e);
}
...
const SWITCHER_CONFIG = {
  ...
  colors: {
    ...
    storyActive: "#ecc94b",
    summaryOn: "#9f7aea",
    summaryActive: "#d6bcfa", // 🎬 新增颜色
    inactive: "#4a5568",
    bg: "#1a202c",
    border: "#2d3748",
    tabActive: "#2b6cb0",
    tabInactive: "transparent",
    floatBg: "rgba(0, 0, 0, 0.55)",
    virgin: "#f687b3",
    nonVirgin: "#9f7aea",
  },
};
...
function getFloatColors() {
  return SWITCHER_STATE.uiDarkMode
    ? {
        ...
        text: "#e2e8f0",
        headBg: "rgba(15,20,30,0.4)",
        headText: "#e2e8f0",
        headBorder: "rgba(255,255,255,0.08)",
        secTitle: "#94a3b8",
        secBorder: "rgba(56,189,248,0.3)",
        itemProBg: "rgba(255,255,255,0.08)",
        itemProText: "#e2e8f0",
    ...
    : {
        bg: "rgba(245,247,250,0.5)",
        border: "rgba(56,189,248,0.5)",
        text: "#1e293b",
        headBg: "rgba(245,247,250,0.5)",
        headText: "#1e293b",
        headBorder: "rgba(15,23,42,0.12)",
        secTitle: "#475569",
        secBorder: "rgba(2,132,199,0.5)",
        itemProBg: "rgba(15,23,42,0.08)",
        itemProText: "#1e293b",
        itemBg: "rgba(15,23,42,0.06)",
        itemText: "#334155",
        more: "#64748b",
        toggle: "#0284c7",
        empty: "#94a3b8",
...
async function getFullContextVar() {
  try {
    ...
    if (
      window.TavernHelper &&
      typeof TavernHelper.getVariables === "function"
    ) {
      vars = await TavernHelper.getVariables({
        type: "message",
        message_id: -1,
      });
...
// 自动蓝灯判定：条目已启用 + 非constant + 剧情显示文本包含其任一关键词
// 命中后由心跳写回为带标记的 constant；失配时仅恢复脚本自动写回的条目。
function checkAutoBlueActivation(entry, displayText) {
  if (!entry || !entry.enabled) return false;
  const strategy = entry.strategy;
  if (!strategy) return false;
  if (strategy.type === "constant") return false; // 手动蓝灯优先，不重复判定
  return isAutoBlueKeywordHit(entry, displayText);
}

function isAutoBlueKeywordHit(entry, displayText) {
  if (!entry || !entry.enabled) return false;
  const strategy = entry.strategy;
  if (!strategy) return false;
  const keys = strategy.keys || [];
    ...
    entries.forEach((entry) => {
      if (entry.name.includes("✍️")) {
        stories.push({
          ...
          simpTrad: getSimpTradType(entry.name),
        });
        return;
      }
      if (entry.name.includes("🎬️")) {
...
async function executeSimpTradSwitch(targetMode, silent = false) {
  try {
    ...
    entries.forEach((entry) => {
      ...
      // 当前模式或无标记的条目：
      if (
        eSimpTrad === targetMode ||
        (eSimpTrad === "none" && entry.name.includes(enableMarker))
      ) {
        const type = getEntryType(entry.name);
        if (type === "pro") {
          ops.push({ uid: entry.uid, enable: false });
...
async function applyChanges(bookName, targetOps, silent = false) {
  ...
  const uidMap = {};
  targetOps.forEach((op) => (uidMap[op.uid] = op.enable));
  try {
...
async function applyStrategyChanges(bookName, strategyOps, silent = false) {
  if (!strategyOps || strategyOps.length === 0) return;
  const strategyMap = {};
  strategyOps.forEach((op) => (strategyMap[op.uid] = op.strategy));
  try {
    await updateWorldbookWith(
      bookName,
      (entries) =>
        entries.map((e) =>
          Object.prototype.hasOwnProperty.call(strategyMap, e.uid)
            ? { ...e, strategy: strategyMap[e.uid] }
            : e,
        ),
      { render: "immediate" },
    );
    ...
    if (!silent) toastr.info(`已更新 ${strategyOps.length} 项触发策略`);
  } catch (e) {
    toastr.error("策略更新失败: " + e.message);
  }
}
    ...
    await updateWorldbookWith(
      ...
      (entries) => {
        return entries.map((entry) => {
          ...
          if (setVirgin) {
            if (hasNonVirgin)
              newContent = newContent.replace(/非处女/g, "处女");
          } else {
            if (hasVirgin)
              newContent = newContent.replace(/(?<!非)处女/g, "非处女");
          }
...
async function togglePinStatus(coreKey) {
  try {
    ...
    if (idx > -1) {
      ...
      await updateVariablesWith(
        (v) => {
          _.set(v, "wuwa_pinned_chars", pinned);
          return v;
        },
        { type: "global" },
      );
...
function shouldAbortChange(localData, ops) {
  ...
  const currentNonPinnedProCount = visiblePairs.filter(
    (p) => p.proEnabled && !p.isPinned,
  ).length;
  ...
  visiblePairs.forEach((p) => {
    ...
    if (
      p.isPinned &&
      p.proUid &&
      changes.hasOwnProperty(p.proUid) &&
      changes[p.proUid] === false
    ) {
  ...
  if (pinnedViolation) {
    console.log(
      `[WuWa Logic] 🛡️ 拦截生效：检测到尝试关闭永久Pro角色，已忽略。`,
    );
    return true;
  }
  ...
  if (nextNonPinnedProCount === 0 && currentNonPinnedProCount > 0) {
    console.log(
      `[WuWa Logic] 🛡️ 拦截生效：检测到所有非永久Pro角色即将离场，已忽略。`,
    );
    return true;
  }
  return false;
...
async function logicScanContext(localData) {
  ...
  try {
    ...
    try {
      ...
      if (
        globals &&
        globals.wuwa_next_event_evicted_at &&
        typeof globals.wuwa_next_event_evicted_at === "object"
      ) {
    ...
    const nextEventText =
      _.get(vars, "stat_data.即将进行的下一个事件节点", "") || "";
    const ops = [];
    const mode = SWITCHER_STATE.simpTradMode;
    ...
    localData.pairs.forEach((char) => {
      ...
      if (char.simpTrad === "simp" && mode === "trad") {
        // 繁体模式下，简体词条保持关闭
        if (char.proEnabled && char.proUid)
          ops.push({ uid: char.proUid, enable: false });
        if (char.liteEnabled && char.liteUid)
          ops.push({ uid: char.liteUid, enable: false });
        return;
      }
      ...
      const proKeys =
        char.proStrategy && Array.isArray(char.proStrategy.keys)
          ? char.proStrategy.keys
          : [];
      ...
      if (proKeys.length > 0) {
        ...
        if (!charData) {
          const fuzzyKey = Object.keys(femaleChars).find(
            (key) => key.includes(charName) || charName.includes(key),
          );
      ...
      if (char.isPinned) {
        ...
        if (nextEventText) {
          if (proKeys.length > 0) {
            mentionedInNextEvent = proKeys.some((key) =>
              nextEventText.includes(key),
            );
          } else {
            mentionedInNextEvent =
              nextEventText.includes(char.coreKey) ||
              nextEventText.includes(char.displayName);
          }

          // [新增] 拦截：如果匹配成功，且当前词条为漂泊者，则检查 NPC漂泊者.是否存在 变量
          if (mentionedInNextEvent) {
            let isRoverEntry =
              char.coreKey.includes("漂泊者") ||
              char.displayName.includes("漂泊者") ||
              char.displayName.includes("男漂") ||
              char.displayName.includes("女漂");
            if (!isRoverEntry && proKeys.length > 0) {
              isRoverEntry = proKeys.some(
                (key) =>
                  key.includes("漂泊者") ||
                  key.includes("男漂") ||
                  key.includes("女漂"),
            ...
            if (isRoverEntry) {
              ...
              if (
                npcRoverVar === false ||
                (typeof npcRoverVar === "string" &&
                  ["false", "no", "0"].includes(
                    npcRoverVar.toLowerCase().trim(),
                  ))
              ) {
              ...
              if (!isNpcRoverExist) {
                ...
                console.log(
                  `[WuWa Logic] 🛡️ 变量显示NPC漂泊者不存在，已拒绝下一节点中的漂泊者匹配`,
                );
              }
            }
          }
        ...
        if (tempLiteChars.includes(char.coreKey)) {
          category = "absentLite";
          const _tl = nextEventSeen.indexOf(char.coreKey);
          if (_tl >= 0) nextEventSeen.splice(_tl, 1);
          if (nextEventEvictedAt.hasOwnProperty(char.coreKey))
            delete nextEventEvictedAt[char.coreKey];
        } else if (mentionedInNextEvent) {
          category = "nextEventCandidate"; // 不在场但被下一事件节点提及（优先于粘性）
          if (!nextEventSeen.includes(char.coreKey))
            nextEventSeen.push(char.coreKey);
          // 重新被提及 → 清除降级计时（重置）
          if (nextEventEvictedAt.hasOwnProperty(char.coreKey))
            delete nextEventEvictedAt[char.coreKey];
          ...
          if (evictedAt && now - evictedAt >= EVICTED_GRACE_MS) {
            ...
            category = "absentLite";
            // 惩罚执行完毕，清除历史标记
            const i1 = nextEventSeen.indexOf(char.coreKey);
            if (i1 >= 0) nextEventSeen.splice(i1, 1);
            if (nextEventEvictedAt.hasOwnProperty(char.coreKey))
              delete nextEventEvictedAt[char.coreKey];
            console.log(
              `[WuWa Logic] ⏰ 降级宽限期到期：${char.displayName} 已强制降为 Lite`,
            );
          } else {
      ...
      if (isPresent) {
        const _i = nextEventSeen.indexOf(char.coreKey);
        if (_i >= 0) nextEventSeen.splice(_i, 1);
        if (nextEventEvictedAt.hasOwnProperty(char.coreKey))
          delete nextEventEvictedAt[char.coreKey];
      }
      ...
      if (!char.proEnabled && nextEventSeen.includes(char.coreKey)) {
        const _j = nextEventSeen.indexOf(char.coreKey);
        if (_j >= 0) nextEventSeen.splice(_j, 1);
        if (nextEventEvictedAt.hasOwnProperty(char.coreKey))
          delete nextEventEvictedAt[char.coreKey];
      }
    ...
    const allManualProChars = allCharInfos.filter(
      (c) => c.category === "manualPro",
    );
    // manualProIndex 越小 = 越晚加入 = 优先级越高 (LIFO)
    allManualProChars.sort(
      (a, b) => a.char.manualProIndex - b.char.manualProIndex,
    );
    ...
    overflowedManualChars.forEach((c) => {
      ...
      allCharInfos.find(
        (info) => info.char.coreKey === c.char.coreKey,
      ).category = "autoCandidate";
    });
    ...
    const autoCandidates = allCharInfos.filter(
      (c) => c.category === "autoCandidate",
    );
    ...
    const nextEventCandidates = allCharInfos.filter(
      (c) => c.category === "nextEventCandidate",
    );
    ...
    //     优先级低于粘性Pro，最易被淘汰
    const evictedNextEventChars = allCharInfos.filter(
      (c) => c.category === "evictedNextEvent",
    );
    ...
    // 5. 不在场但当前是Pro的粘性角色
    //    排序规则：好感度从高到低；同好感度按名字 A→Z（淘汰时从 Z 开始淘汰）
    const absentStickyChars = allCharInfos.filter(
      (c) => c.category === "absentSticky",
    );
    ...
    const absentLiteChars = allCharInfos.filter(
      (c) => c.category === "absentLite",
    );
    ...
    const autoProChars = mergedProChars.filter(
      (c) => c.category === "autoCandidate",
    );
    const autoLiteCandidates = mergedLiteCandidates.filter(
      (c) => c.category === "autoCandidate",
    );
    let nextEventProChars = mergedProChars.filter(
      (c) => c.category === "nextEventCandidate",
    );
    let nextEventLiteChars = mergedLiteCandidates.filter(
      (c) => c.category === "nextEventCandidate",
    );
    ...
    const evictedNextRemainingSlots = Math.max(
      0,
      stickySlots - stickyProChars.length,
    );
    const evictedNextProChars = evictedNextEventChars.slice(
      0,
      evictedNextRemainingSlots,
    );
    const evictedNextLiteChars = evictedNextEventChars.slice(
      evictedNextRemainingSlots,
    );
    ...
    allCharInfos.forEach((info) => {
      if (
        tempLiteChars.includes(info.char.coreKey) &&
        !info.isPresent &&
        tempLiteSeenPresent.includes(info.char.coreKey)
      ) {
    ...
    allCharInfos.forEach((info) => {
      if (
        info.char.isManualPro &&
        !info.isPresent &&
        manualProSeenPresent.includes(info.char.coreKey)
      ) {
    ...
    if (tempLiteToRemove.length > 0) {
      tempLiteChars = tempLiteChars.filter(
        (k) => !tempLiteToRemove.includes(k),
      );
      tempLiteSeenPresent = tempLiteSeenPresent.filter(
        (k) => !tempLiteToRemove.includes(k),
      );
      updateVars.wuwa_temp_lite_chars = tempLiteChars;
      updateVars.wuwa_temp_lite_seen_present = tempLiteSeenPresent;
      console.log(
        "[WuWa Logic] 🧹 暂时Lite清理：已移除离场角色",
        tempLiteToRemove,
      );
    } else {
    ...
    try {
      ...
      const existingTempLiteSeen =
        gl && Array.isArray(gl.wuwa_temp_lite_seen_present)
          ? gl.wuwa_temp_lite_seen_present
          : [];
      const hasNewTempLiteSeen = tempLiteSeenPresent.some(
        (k) => !existingTempLiteSeen.includes(k),
      );
      const hasRemovedTempLiteSeen = existingTempLiteSeen.some(
        (k) => !tempLiteSeenPresent.includes(k),
      );
    ...
    if (manualProToRemove.length > 0) {
      ...
      updateVars.wuwa_manual_pro_chars = currentManualPro.filter(
        (k) => !manualProToRemove.includes(k),
      );
    ...
    try {
      const gl = await getVariables({ type: "global" });
      const existing =
        gl && Array.isArray(gl.wuwa_manual_pro_seen_present)
          ? gl.wuwa_manual_pro_seen_present
          : [];
      // 计算最终值：本轮累积的 seen_present 减去需要移除的
      const finalSeenPresent = manualProSeenPresent.filter(
        (k) => !manualProToRemove.includes(k),
      );
      // 判断是否有变化（新增或移除）
      const hasNewEntries = finalSeenPresent.some((k) => !existing.includes(k));
      const hasRemovedEntries = existing.some(
        (k) => !finalSeenPresent.includes(k),
      );
    ...
    try {
      ...
      console.warn("[WuWa Logic] nextEventSeen/evictedAt 持久化失败:", e);
    }
        ...
        await updateVariablesWith(
          (v) => {
            for (const k in updateVars) {
              if (
                updateVars[k] === null ||
                (Array.isArray(updateVars[k]) && updateVars[k].length === 0)
              ) {
                _.unset(v, k);
              } else {
    ...
    allCharInfos.forEach((info) => {
      ...
      if (shouldPro) {
        if (!char.proEnabled && char.proUid)
          ops.push({ uid: char.proUid, enable: true });
        if (char.liteEnabled && char.liteUid)
          ops.push({ uid: char.liteUid, enable: false });
      } else {
        if (!char.liteEnabled && char.liteUid)
          ops.push({ uid: char.liteUid, enable: true });
        if (char.proEnabled && char.proUid)
          ops.push({ uid: char.proUid, enable: false });
      }
    });
    ...
    console.error("[WuWa Logic] Variable scan error:", error);
  }
}
...
async function logicScanInput(localData) {
  ...
  try {
    ...
    console.log(
      "[WuWa Logic] 🧹 开场指令触发，已清空手动Pro队列、暂时Lite名单、已见在场记录、下一节点历史和降级计时",
    );
  } catch (e) {}
  ...
  if (charMatch && charMatch[1]) {
    ...
    localData.pairs.forEach((p) => {
      ...
      // 读取 Pro 条目的关键词列表
      const proKeys =
        p.proStrategy && Array.isArray(p.proStrategy.keys)
          ? p.proStrategy.keys
          : [];
      if (proKeys.length > 0) {
        ...
        isTarget = targets.some(
          (t) => p.displayName.includes(t) || p.coreKey.includes(t),
        );
      }
    ...
    if (ops.length > 0) {
      ...
      console.log(
        `[WuWa Logic] 🎯 开场指令接管：互动角色 [${targets.join(", ")}]，共${ops.length}条更新（不受Pro上限约束）`,
      );
      await applyChanges(localData.pairs[0].bookName, ops, true);
    }
    return true;
...
async function logicScanFeixun(localData) {
  ...
  const isActive =
    fxShared && fxShared.currentChat && fxShared.currentChat.trim() !== "";
  ...
  if (isActive) {
    ...
    localData.pairs.forEach((p) => {
      ...
      if (proKeys.length > 0) {
        ...
        isTarget =
          targetStr.includes(p.coreKey) || targetStr.includes(p.displayName);
      }

      if (isTarget) {
        if (!p.proEnabled && p.proUid)
          ops.push({ uid: p.proUid, enable: true });
        if (p.liteEnabled && p.liteUid)
          ops.push({ uid: p.liteUid, enable: false });
      } else {
        // 非目标角色：直接关闭Pro（永久Pro除外），实现绝对覆盖
        if (!p.isPinned) {
          if (!p.liteEnabled && p.liteUid)
            ops.push({ uid: p.liteUid, enable: true });
          if (p.proEnabled && p.proUid)
            ops.push({ uid: p.proUid, enable: false });
        }
      }
    });
    ...
    if (feixunActive) {
      ...
      localData.pairs.forEach((p) => {
        ...
        if (shouldPro) {
          if (!p.proEnabled && p.proUid)
            restoreOps.push({ uid: p.proUid, enable: true });
          if (p.liteEnabled && p.liteUid)
            restoreOps.push({ uid: p.liteUid, enable: false });
        } else {
          // 不在快照中的角色必须关闭Pro（除非是永久Pro，已跳过）
          if (!p.liteEnabled && p.liteUid)
            restoreOps.push({ uid: p.liteUid, enable: true });
          if (p.proEnabled && p.proUid)
            restoreOps.push({ uid: p.proUid, enable: false });
        }
      });
...
async function logicScanFloorZero(localData) {
  try {
    ...
    console.log(
      "[WuWa Logic] 🏠 楼层0强制同步：已清空手动Pro队列、暂时Lite名单、已见在场记录、下一节点历史和降级计时",
    );
...
async function masterLoop() {
  ...
  if (
    ...
    (!currentInput ||
      ...
      !currentInput.includes("[系统指令：生成开场剧情]"))
  ) {
...
async function floatScanAndRefresh(data, scanText, displayText) {
  try {
    ...
    const activeStories = data.stories.filter(
      (s) =>
        s.enabled &&
        matchSimpTradMode(s.simpTrad, _mode) &&
        getStoryActivationState(s, scanText, displayText).active,
    );
    const activeSummaries = data.summaries.filter(
      (s) =>
        s.enabled &&
        matchSimpTradMode(s.simpTrad, _mode) &&
        getStoryActivationState(s, scanText, displayText).active,
...
function createFloatingWindow() {
  ...
  const sizeTitle =
    SWITCHER_STATE.floatSizeMode === "large"
      ? "切换为小悬浮窗"
      : "切换为大悬浮窗 (1.5x)";
  ...
  if (typeof $floatWin.draggable === "function") {
    $floatWin.draggable({
      cancel:
        "#wb-float-size-toggle, #wb-float-close, #wb-float-dark-toggle, .wb-float-toggle-expand",
      containment: "window",
      start: function () {
        isDragging = true;
      },
      stop: function (event, ui) {
        SWITCHER_STATE.floatPos = {
  ...
  $("#wb-float-dark-toggle").on("click", async function (e) {
    ...
    if (typeof toastr !== "undefined")
      toastr.info(
        SWITCHER_STATE.uiDarkMode ? "已切换为深色模式" : "已切换为浅色模式",
      );
  });
  $("#wb-float-size-toggle").on("click", function (e) {
    ...
    if (sizeBtn.length) {
      ...
      sizeBtn.attr(
        "title",
        isLarge ? "切换为小悬浮窗" : "切换为大悬浮窗 (1.5x)",
      );
      sizeBtn.text(isLarge ? "📏" : "📐");
    }
    toastr.info(
      isLarge ? "悬浮窗已切换为大模式 (1.5x)" : "悬浮窗已切换为小模式",
    );
  });
...
async function refreshFloatingWindowContent() {
  ...
  if (res.success) {
    ...
    const activeStories = res.stories.filter(
      (s) =>
        s.enabled &&
        matchSimpTradMode(s.simpTrad, _fmode) &&
        getStoryActivationState(s, scanText, displayText).active,
    );
    const activeSummaries = res.summaries.filter(
      (s) =>
        s.enabled &&
        matchSimpTradMode(s.simpTrad, _fmode) &&
        getStoryActivationState(s, scanText, displayText).active,
    ...
    if (proList.length > 0) {
      listEl.append(
        `<div style='font-size:${fs9}px;color:${FC.secTitle};font-weight:bold;border-bottom:1px dashed ${FC.secBorder};padding-bottom:1px;margin-bottom:1px;'>🟢 Pro(${proList.length})</div>`,
      );
      ...
      showList.forEach((p) =>
        listEl.append(
          `<div style='padding:${pad13};background:${FC.itemProBg};border-left:2px solid #48bb78;border-radius:2px;color:${FC.itemProText};overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-bottom:1px;font-size:${fs9}px;'>${p.displayName}</div>`,
        ),
      );

      if (proList.length > maxShow) {
        listEl.append(
          `<div style='padding:${pad13};color:${FC.more};font-size:${fs8}px;font-style:italic;'>+${proList.length - maxShow}</div>`,
        );
      }
      ...
      if (proList.length > COLLAPSED_MAX) {
        ...
        listEl.append(
          `<div class='wb-float-toggle-expand' style='padding:${pad13};color:${FC.toggle};font-size:${fs8}px;text-align:center;cursor:pointer;user-select:none;'>${toggleText}</div>`,
        );
      }
    } else {
      listEl.append(
        `<div style='font-size:${fs9}px;color:${FC.empty};text-align:center;padding:3px;'>无Pro</div>`,
      );
    }
    ...
    if (activeStories.length > 0) {
      listEl.append(
        `<div style='font-size:${fs9}px;color:${FC.secTitle};font-weight:bold;border-bottom:1px dashed ${FC.secBorder};padding-bottom:1px;margin-bottom:1px;margin-top:3px;'>✍️ 剧情(${activeStories.length})</div>`,
      );
        ...
        const borderColor =
          stState.mode === "manualBlue"
            ...
            : stState.mode === "autoBlue"
              ...
              : "#ecc94b";
        listEl.append(
          `<div style='padding:${pad13};background:${FC.itemBg};border-left:2px solid ${borderColor};border-radius:2px;color:${FC.itemText};overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-bottom:1px;font-size:${fs9}px;'>${icon}${s.name.replace("✍️", "").trim()}</div>`,
        );
      });
    }
    ...
    if (activeSummaries.length > 0) {
      listEl.append(
        `<div style='font-size:${fs9}px;color:${FC.secTitle};font-weight:bold;border-bottom:1px dashed ${FC.secBorder};padding-bottom:1px;margin-bottom:1px;margin-top:3px;'>🎬 梗概(${activeSummaries.length})</div>`,
      );
        ...
        const sumBorderColor =
          stState.mode === "manualBlue"
            ...
            : stState.mode === "autoBlue"
              ...
              : "#9f7aea";
        listEl.append(
          `<div style='padding:${pad13};background:${FC.itemBg};border-left:2px solid ${sumBorderColor};border-radius:2px;color:${FC.itemText};overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-bottom:1px;font-size:${fs9}px;'>${icon}${s.name.replace("🎬️", "").trim()}</div>`,
        );
      });
    }
    ...
    $("#wb-float-list")
      .off("click.proexpand")
      .on("click.proexpand", ".wb-float-toggle-expand", function (e) {
        e.stopPropagation();
        e.preventDefault();
...
function createSwitcherPanel() {
  ...
  const autoBtnColor = SWITCHER_STATE.autoMode
    ? SWITCHER_CONFIG.colors.pro
    : SWITCHER_CONFIG.colors.inactive;
  ...
  $("#wb-simp-trad-toggle").on("click", async function () {
    ...
    if (
      !confirm(
        "⚠️ Cyberpunk 2077 官方简中/繁中翻译差异较大\n\n" +
          "简体模式：使用赛博朋克官方简体中文译名\n\n" +
          "繁体模式：使用電馭叛客官方繁體中文譯名\n\n" +
          "繁体模式下会同时启用游戏术语纠正提示词。\n\n" +
  ...
  $("#wb-switcher-search").on("input", function () {
    ...
    $(".wb-item-row").each(function () {
      $(this).toggle(
        $(this).data("name").toString().toLowerCase().includes(val),
      );
    });
  });
  ...
  $("#wb-tab-others").on("click", () => switchTab("others"));

  $("#wb-toggle-float").on("click", function () {
    SWITCHER_STATE.floatVisible = !SWITCHER_STATE.floatVisible;
    saveSettings();
  ...
  $("#wb-toggle-auto").on("click", async function () {
    ...
    if (floatBtn.length) {
      ...
      floatBtn.attr(
        "title",
        SWITCHER_STATE.autoMode ? "自动模式: ON" : "自动模式: OFF",
      );
    }
  ...
  $("#wb-settings-btn").on("click", async function () {
    ...
    const input = prompt(
      '⚙️ 请输入同时存在的最大 Pro 词条数量:\n(注意：⭐永久Pro不占名额；该上限仅对"环境在场"扫描生效，不会拦截飞讯和指令。)',
      currentMax,
    );
...
let currentData = { pairs: [], stories: [], summaries: [] },
  currentView = "chars";
async function loadDataAndRender() {
  const result = await scanAndPairEntries();
  $("#wb-current-book-display")
    .text(
      result.bookName
        ? `当前绑定世界书: ${result.bookName}`
        : "⚠️ 未检测到绑定的世界书",
    )
    .css("color", result.bookName ? "#63b3ed" : "#fc8181");
  if (result.success) {
    currentData = {
      pairs: result.pairs,
    ...
    $("#wb-switcher-list").html(
      `<div style='text-align:center;color:#fc8181;padding:20px;'>${result.message}</div>`,
    );
}
...
async function switchTab(view) {
  currentView = view;
  const active = {
    background: SWITCHER_CONFIG.colors.tabActive,
    color: "white",
  };
  const inactive = {
    background: SWITCHER_CONFIG.colors.tabInactive,
    color: "#a0aec0",
  };
  ...
  $("#wb-tab-others").css(view === "others" ? active : inactive);

  if (view === "others") {
    $("#wb-virgin-btns").hide();
    $("#wb-switcher-search").hide();
    await renderOtherEntriesView();
  } else {
    $("#wb-global-btns").show();
...
function renderGlobalButtonsChars() {
  ...
  $("#wb-global-pro").on("click", async () => {
    ...
    if (!visible.length)
      return toastr.warning("当前模式下没有可操作的角色词条");
    const ops = [];
    visible.forEach((i) => {
      if (i.proUid) ops.push({ uid: i.proUid, enable: true });
      if (i.liteUid) ops.push({ uid: i.liteUid, enable: false });
    });
    await applyChanges(visible[0].bookName, ops);
    toastr.success("全部 Pro 模式");
    loadDataAndRender();
  });
  $("#wb-global-lite").on("click", async () => {
    ...
    const ops = [];
    visible.forEach((i) => {
      if (i.liteUid) ops.push({ uid: i.liteUid, enable: true });
      if (i.proUid) ops.push({ uid: i.proUid, enable: false });
    });
    await applyChanges(visible[0].bookName, ops);
    toastr.success("全部 Lite 模式");
    loadDataAndRender();
  });
  ...
  $("#wb-global-virgin").on("click", async () => {
    ...
    if (modifiable.length === 0)
      return toastr.warning("未找到包含“处女”设定的角色");
...
function renderListChars() {
  ...
  if (currentData.pairs.length === 0)
    return list.html(
      `<div style='text-align:center;color:#718096;padding:20px;'>未找到 [Pro]/[Lite] 角色</div>`,
    );
  ...
  visiblePairs.forEach((item, idx) => {
    const isPro = item.proEnabled && !item.liteEnabled;
    const isLite = !item.proEnabled && item.liteEnabled;
    let virginBtnHtml = "";
    if (item.virginModifiable) {
      const vColor = item.isVirgin
        ? SWITCHER_CONFIG.colors.virgin
        : SWITCHER_CONFIG.colors.nonVirgin;
  ...
  $(".wb-btn-pro").on("click", async function () {
    ...
    if (SWITCHER_STATE.autoMode) {
      ...
      if (i.proEnabled && !i.liteEnabled) {
        ...
        await updateVariablesWith(
          (v) => {
            _.set(v, "wuwa_manual_pro_chars", manualProChars);
            _.set(v, "wuwa_temp_lite_chars", tempLiteChars);
            return v;
          },
          { type: "global" },
        );
        await applyChanges(
          i.bookName,
          [
            { uid: i.liteUid, enable: true },
            i.proUid ? { uid: i.proUid, enable: false } : null,
          ].filter(Boolean),
        );
        $(this)
          .closest(".wb-item-row")
          .find(".wb-btn-pro")
          .css({ background: "transparent", color: "#a0aec0" });
        $(this)
          .closest(".wb-item-row")
          .find(".wb-btn-lite")
      ...
      if (nonPinnedProCount >= maxProCount) {
        toastr.warning(
          `⚠️ 已达到Pro上限(${maxProCount}个)，请先将其他角色切换为Lite，或修改设置中的最大上限`,
        );
        return;
      }
    ...
    await applyChanges(
      i.bookName,
      [
        { uid: i.proUid, enable: true },
        i.liteUid ? { uid: i.liteUid, enable: false } : null,
      ].filter(Boolean),
    );
    $(this)
      .closest(".wb-item-row")
      .find(".wb-btn-pro")
      .css({ background: SWITCHER_CONFIG.colors.pro, color: "white" });
    $(this)
      .closest(".wb-item-row")
      .find(".wb-btn-lite")
      .css({ background: "transparent", color: "#a0aec0" });
    ...
    if (SWITCHER_STATE.autoMode) {
      ...
      if (rescan.success) {
        const nonPinnedPros = rescan.pairs.filter(
          (p) => p.proEnabled && !p.isPinned,
        );
        let maxPC = 3;
        try {
          const gb = await getVariables({ type: "global" });
          if (gb && gb.wuwa_max_pro_count !== undefined)
            maxPC = parseInt(gb.wuwa_max_pro_count, 10);
        } catch (e) {}
        if (nonPinnedPros.length > maxPC) {
          console.log(
            `[WuWa Logic] ⚠️ 检测到Pro超限(${nonPinnedPros.length}/${maxPC})，触发淘汰...`,
          );
          await logicScanContext({
            pairs: rescan.pairs,
            stories: rescan.stories,
            summaries: rescan.summaries,
          });
        }
      }
      masterLoop();
...
function renderGlobalButtonsStories() {
  ...
  const _allSVisible = currentData.stories.filter((s) =>
    matchSimpTradMode(s.simpTrad, _allSMode),
  );
  $("#wb-story-all-on").on("click", async () => {
    if (!_allSVisible.length) return;
    await applyChanges(
      _allSVisible[0].bookName,
      _allSVisible.map((s) => ({ uid: s.uid, enable: true })),
    );
    toastr.success("已开启当前模式所有剧情");
    loadDataAndRender();
  });
  $("#wb-story-all-off").on("click", async () => {
    if (!_allSVisible.length) return;
    await applyChanges(
      _allSVisible[0].bookName,
      _allSVisible.map((s) => ({ uid: s.uid, enable: false })),
    );
    toastr.success("已关闭当前模式所有剧情");
    loadDataAndRender();
  });
}
...
function renderGlobalButtonsSummaries() {
  ...
  const _allSumVisible = currentData.summaries.filter((s) =>
    matchSimpTradMode(s.simpTrad, _allSumMode),
  );
  $("#wb-summary-all-on").on("click", async () => {
    if (!_allSumVisible.length) return;
    await applyChanges(
      _allSumVisible[0].bookName,
      _allSumVisible.map((s) => ({ uid: s.uid, enable: true })),
    );
    toastr.success("已开启当前模式所有梗概");
    loadDataAndRender();
  });
  $("#wb-summary-all-off").on("click", async () => {
    if (!_allSumVisible.length) return;
    await applyChanges(
      _allSumVisible[0].bookName,
      _allSumVisible.map((s) => ({ uid: s.uid, enable: false })),
    );
    toastr.success("已关闭当前模式所有梗概");
    loadDataAndRender();
  });
}
...
async function renderListStories() {
  ...
  const visibleStories = currentData.stories.filter((s) =>
    matchSimpTradMode(s.simpTrad, _smode),
  );
  if (visibleStories.length === 0)
    return list.html(
      `<div style='text-align:center;color:#718096;padding:20px;'>未找到当前模式下的剧情条目(需包含✍️)</div>`,
    );
  ...
  visibleStories.forEach((item, idx) => {
    ...
    let nameStyle =
      "font-weight:bold;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-right:10px;";
  ...
  $(".wb-btn-story-toggle").on("click", async function () {
    ...
    $(this)
      .css(
        "background",
        s ? SWITCHER_CONFIG.colors.storyOn : SWITCHER_CONFIG.colors.inactive,
      )
      .text(s ? "ON" : "OFF");
  });
...
async function renderListSummaries() {
  ...
  const visibleSummaries = currentData.summaries.filter((s) =>
    matchSimpTradMode(s.simpTrad, _smode2),
  );
  if (visibleSummaries.length === 0)
    return list.html(
      `<div style='text-align:center;color:#718096;padding:20px;'>未找到当前模式下的梗概条目(需包含🎬️)</div>`,
    );
  ...
  $(".wb-btn-summary-toggle").on("click", async function () {
    ...
    $(this)
      .css(
        "background",
        s ? SWITCHER_CONFIG.colors.summaryOn : SWITCHER_CONFIG.colors.inactive,
      )
      .text(s ? "ON" : "OFF");
  });
...
function saveSettings() {
  localStorage.setItem(
    SWITCHER_CONFIG.storageKey,
    JSON.stringify({
      autoMode: SWITCHER_STATE.autoMode,
      floatVisible: SWITCHER_STATE.floatVisible,
      floatPos: SWITCHER_STATE.floatPos,
      simpTradMode: SWITCHER_STATE.simpTradMode,
      floatSizeMode: SWITCHER_STATE.floatSizeMode,
    }),
  );
}
function loadSettings() {
  try {
    const s = JSON.parse(localStorage.getItem(SWITCHER_CONFIG.storageKey));
    if (s) SWITCHER_STATE = { ...SWITCHER_STATE, ...s };
  } catch (e) {}
}
...
// ============================================================
// 📖 "其他条目" 模块 (v4.1)
// 按 emoji 自动分类世界书条目：一级类别 + 二级地区，两级折叠 + 双列 + 单条开关
// 分类按【首码点】匹配，兼容 VS16 有无
// ============================================================

const OTHER_ENTRIES_CONFIG = {
  // 一级类别：key 用真实 emoji，_other_cpLookup 按首码点匹配（兼容 VS16 有无）
  L1_CATEGORIES: {
    "📚": "📚 设定",
    "🗺️": "🗺️ 地区",
    "👥": "👥 组织",
...
const OTHER_ENTRIES_STATE = {
  allEntries: [],
  classified: [],
  triggeredUids: new Set(),
  triggeredTokenCount: null,
...
function _other_cpLookup(map, tok) {
  ...
  const cp = _other_firstCP(tok);
  for (const k in map) { if (_other_firstCP(k) === cp) return map[k]; }
  return undefined;
}

function _other_isEmojiStart(cp) {
...
// 取条目名开头的 emoji token 序列
function _other_splitLeadingEmojis(str) {
  const result = [];
  let i = 0;
  const n = str.length;
  while (i < n && result.length < 4) {
    const cp = str.codePointAt(i);
    if (!_other_isEmojiStart(cp)) break;
    const start = i;
    i += cp > 0xffff ? 2 : 1;
    while (i < n) {
      const nx = str[i];
      const cc = str.codePointAt(i);
      if (nx === "\u200D") { i += cc > 0xffff ? 2 : 1; if (i < n) { const c2 = str.codePointAt(i); i += c2 > 0xffff ? 2 : 1; } continue; }
      if (nx === "\uFE0F") { i += 1; continue; }
      if (cc >= 0x1f3fb && cc <= 0x1f3ff) { i += 1; continue; }
      if (cc === 0x2640 || cc === 0x2642) { i += cc > 0xffff ? 2 : 1; continue; }
      break;
    }
...
function _other_cleanName(name) {
  return name
...
function _other_classifyEntry(entry) {
  const name = entry.name || "";
  const toks = _other_splitLeadingEmojis(name);
  const cleanName = _other_cleanName(name);
  if (toks.length === 0) return { excluded: "系统/变量", cleanName };
  ...
  if (ex) return { excluded: ex, cleanName };
  let l1 = null, l2 = null;
  // 杂项
  ...
  // 独立一级
  if (_other_cpLookup(OTHER_ENTRIES_CONFIG.L1_CATEGORIES, e1)) {
    l1 = _other_cpLookup(OTHER_ENTRIES_CONFIG.L1_CATEGORIES, e1);
...
async function _other_loadAndRender() {
  ...
  try {
    var bookName = null;
    try {
      var charBooks = getCharWorldbookNames("current");
      if (charBooks && charBooks.primary) bookName = charBooks.primary;
    } catch (e) {}
    if (!bookName) { if (list) list.innerHTML = "<div style='text-align:center;color:#fc8181;padding:20px;'>⚠️ 未检测到绑定的世界书</div>"; return; }
    var entries = await getWorldbook(bookName);
    if (!entries || entries.length === 0) { if (list) list.innerHTML = "<div style='text-align:center;color:#fc8181;padding:20px;'>世界书为空</div>"; return; }

    OTHER_ENTRIES_STATE.allEntries = entries.map(function (e) {
      return { uid: e.uid, name: e.name, enabled: e.enabled, bookName: bookName };
    });
    ...
    console.error("[WuWa OtherEntries] render failed:", e);
    if (list) list.innerHTML = "<div style='text-align:center;color:#fc8181;padding:20px;'>加载失败: " + e.message + "</div>";
  }
}

function _other_renderList() {
  var st = OTHER_ENTRIES_STATE;
  var data = st.classified;
  ...
  for (var i = 0; i < data.length; i++) {
    var x = data[i];
    if (!byL1[x.l1]) byL1[x.l1] = {};
    if (!byL1[x.l1][x.l2]) byL1[x.l1][x.l2] = [];
    byL1[x.l1][x.l2].push(x);
  }
  var kw = (st._searchKw || "").trim().toLowerCase();
  function vis(item) {
    if (kw && item.cleanName.toLowerCase().indexOf(kw) < 0 && (item.entry.name || "").toLowerCase().indexOf(kw) < 0) return false;
    if (st.showOnlyTriggered && !st.triggeredUids.has(item.entry.uid)) return false;
    return true;
  }

  var parts = [];
  var hasAny = false;
  for (var li = 0; li < l1Order.length; li++) {
    var l1 = l1Order[li];
    var l2map = byL1[l1];
    if (!l2map) continue;
    ...
    if (visInL1.length === 0) continue;
    hasAny = true;
    var innerParts = [];
    for (var lk2 = 0; lk2 < l2keys.length; lk2++) {
      var l2 = l2keys[lk2];
      var items = l2map[l2].filter(vis);
...
function _other_renderItem(item, C) {
  var e = item.entry;
  var on = e.enabled;
  var trig = OTHER_ENTRIES_STATE.triggeredUids.has(e.uid);
  var bg = trig ? "rgba(236,201,75,0.18)" : "rgba(255,255,255,0.03)";
...
function _other_bindEvents() {
  ...
  // 单条开关
  $("#wb-switcher-list").off("click", ".wb-other-item").on("click", ".wb-other-item", async function (e) {
    e.stopPropagation();
...
function _other_updateRowVisual(row, entry) {
  var trig = OTHER_ENTRIES_STATE.triggeredUids.has(entry.uid);
  var on = entry.enabled;
  var C = SWITCHER_CONFIG.colors;
  row.css("background", trig ? "rgba(236,201,75,0.18)" : "rgba(255,255,255,0.03)");
  row.find("span").first().css("color", on ? "#e2e8f0" : "#718096");
  row.find("span").last().css("background", on ? (trig ? C.storyActive : C.pro) : C.inactive);
}

// ===== 触发检测（移植自 Arknights sendInterceptor 干跑思路，只读预览）=====
async function _other_runDetection() {
  var st = OTHER_ENTRIES_STATE;
  st.isDetecting = true;
  st.triggeredUids = new Set();
  st.triggeredTokenCount = null;
  _other_renderList();
  try {
    var ctx = (typeof getContext === "function") ? getContext() : (typeof SillyTavern !== "undefined" && SillyTavern.getContext) ? SillyTavern.getContext() : null;
    var getWI = ctx && ctx.getWorldInfoPrompt;
    if (!getWI) { toastr.error("无法获取 getWorldInfoPrompt 接口"); return; }

    var chat = (ctx && ctx.chat) ? ctx.chat.slice() : [];
    var mock = chat.map(function (m) {
      if (typeof m === "string") return m;
      if (m && typeof m.mes !== "undefined") {
        var nm = m.name;
        if (!nm && typeof SillyTavern !== "undefined") nm = m.is_user ? SillyTavern.name1 : SillyTavern.name2;
        return nm ? (nm + ": " + m.mes) : String(m.mes);
      }
      return String(m);
    });
    var inputEl = (window.parent && window.parent.document) ? window.parent.document.querySelector("#send_textarea") : document.querySelector("#send_textarea");
    var inputText = inputEl ? (inputEl.value || "").trim() : "";
    if (inputText) {
      var uname = (typeof SillyTavern !== "undefined") ? SillyTavern.name1 : "User";
      mock.push(uname + ": " + inputText);
    }
    mock.reverse();
    mock.__isMock = true;

    var activatedUids = new Set();
    var collector = function (ev) {
      var detail = ev.detail || ev;
      if (Array.isArray(detail)) { for (var i = 0; i < detail.length; i++) { if (detail[i] && detail[i].uid != null) activatedUids.add(detail[i].uid); } }
    };
    var hostDoc = (window.parent && window.parent.document) ? window.parent.document : document;
    hostDoc.addEventListener("world_info_activated", collector);
    if (typeof eventOn === "function") eventOn("world_info_activated", collector);

    var timeoutErr = new Error("DRY_RUN_TIMEOUT");
    try {
      await Promise.race([
        getWI.call(ctx, mock, 1000000, false, { trigger: "wuwa_other_dry_run" }),
        new Promise(function (_, rej) { setTimeout(function () { rej(timeoutErr); }, 10000); }),
      ]);
    } catch (e) {
      if (e === timeoutErr) { toastr.error("世界书检测超时（10s）"); return; }
      console.error("[WuWa OtherEntries] WI dry run failed:", e);
      toastr.error("世界书检测出错：" + e.message);
      return;
    } finally {
      hostDoc.removeEventListener("world_info_activated", collector);
      if (typeof eventOff === "function") { try { eventOff("world_info_activated", collector); } catch (e2) {} }
    }

    st.triggeredUids = activatedUids;
    st.triggeredTokenCount = await _other_estimateTokens(ctx, inputText);
...
async function _other_estimateTokens(ctx, inputText) {
  try {
    var generate = ctx && ctx.generate;
    if (generate && generate.name === "safeGenerate") {
      return await _other_fastEstimate(inputText);
    }
    var tokenCount = null;
    var lastText = "";
    var onReady = function (ev) {
      var d = ev.detail || ev;
      var arr = (d && (d.chat || d.prompt)) || [];
      lastText = Array.isArray(arr) ? arr.map(function (x) { return (typeof x === "object") ? (x.content || (x.name + ": " + x.mes)) : String(x); }).join("\n") : String(arr);
    };
    var hostDoc = (window.parent && window.parent.document) ? window.parent.document : document;
    hostDoc.addEventListener("chat_completion_prompt_ready", onReady);
    if (typeof eventOn === "function") eventOn("chat_completion_prompt_ready", onReady);
    try {
      await Promise.race([
        generate ? generate("normal", {}, true) : Promise.reject(new Error("no generate")),
        new Promise(function (_, rej) { setTimeout(function () { rej(new Error("GEN_TIMEOUT")); }, 15000); }),
      ]);
    } catch (e) {
      if (String(e.message) === "GEN_TIMEOUT") return "计算超时";
      return await _other_fastEstimate(inputText);
    } finally {
      hostDoc.removeEventListener("chat_completion_prompt_ready", onReady);
      if (typeof eventOff === "function") { try { eventOff("chat_completion_prompt_ready", onReady); } catch (e2) {} }
    }
    if (typeof SillyTavern !== "undefined" && typeof SillyTavern.getTokenCountAsync === "function" && lastText) {
      return await SillyTavern.getTokenCountAsync(lastText);
    }
    return "已检测";
  } catch (e) {
    console.error("[WuWa OtherEntries] token estimate failed:", e);
    return "计算失败";
  }
}

async function _other_fastEstimate(inputText) {
  var text = "";
